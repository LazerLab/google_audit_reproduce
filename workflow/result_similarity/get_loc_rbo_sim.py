...
def get_dataframe(path, qry, day, district1, district2):
    ...
    # standardize loc id
    df["loc_id"] = np.where(df["loc_id"] == 'Congressional District 1,Hawaii,United States', 'HI-1,Hawaii,United States', df["loc_id"])
    df["loc_id"] = np.where(df["loc_id"] == 'Congressional District 6,California,United States', 'CA-6,California,United States', df["loc_id"])
    df["loc_id"] = np.where(df["loc_id"] == 'Congressional District 45,California,United States', 'CA-45,California,United States', df["loc_id"])
...
def get_results(qry_loc_checkhome, path):
    ...
    for qry in qry_list:
        ...
        homedist = qry_set[qry_set["is_home_district"]==True]['loc_id_clean'].drop_duplicates().to_list()[0] # home district
        loc_list = qry_set[qry_set["is_home_district"]==False]['loc_id_clean'].drop_duplicates().to_list()
